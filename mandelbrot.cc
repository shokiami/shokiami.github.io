...
const vector<png::rgb_pixel> palette = {
  {66, 30, 15}, {25, 7, 26}, {9, 1, 47}, {4, 4, 73}, {0, 7, 100}, {1, 44, 138}, {24, 82, 177}, {57, 125, 209},
    ...
    for (int j = 0; j < WIDTH; j++) {
      ...
      for (int itr = 0; itr < MAX_ITR; itr++) {
        ...
        if (z_r2 + z_i2 > BAILOUT * BAILOUT) {
          double p = min(max(1.0 - log2(0.5 * log(z_r2 + z_i2) / log(BAILOUT)), 0.0), 1.0);
          png::rgb_pixel color_1 = palette[itr % palette.size()];
          png::rgb_pixel color_2 = palette[(itr + 1) % palette.size()];
...
int main() {
  render(0.0, 0.0, 1.0, "mandelbrot_set.png");
